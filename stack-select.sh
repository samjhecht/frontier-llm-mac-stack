...
select_stack() {
    ...
    # Create main docker-compose.yml with a more compatible approach
    # We'll use docker-compose with multiple -f flags instead of include
    cat > "${SCRIPT_DIR}/docker-compose.yml" << EOF
# Auto-generated docker-compose.yml - DO NOT EDIT
# Generated by stack-select.sh for stack: ${stack_name}
# To change stacks, run: ./stack-select.sh <stack-name>
# 
# This is a placeholder file. The actual configuration is loaded via:
# docker-compose -f stacks/${stack_name}/docker-compose.yml \\
#                -f stacks/common/monitoring/docker-compose.yml \\
#                -f stacks/common/nginx/docker-compose.yml
...
services:
  placeholder:
    image: alpine:latest
    command: echo "Please use ./start.sh to run the stack"
    profiles:
      - never-run
EOF

    # Create a docker-compose wrapper script
    cat > "${SCRIPT_DIR}/docker-compose-wrapper.sh" << 'EOF'
#!/bin/bash
